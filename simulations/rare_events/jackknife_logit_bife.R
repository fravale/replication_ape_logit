...
jackknife_logit <- function(id, y, X, jk = TRUE, dyn = FALSE){
    ...
    bform = as.formula(paste("y ~ ", paste(paste0("V", 3:(ncol(datibife))), collapse = " + "),paste("| id")))
    ...
    if(jk){
        ...
        for(t in 1:G){
            ...
            bjkform = as.formula(paste("yj ~ ", paste(paste0("V", 3:(ncol(datibifejk))), collapse = " + "),paste("| idj")))
