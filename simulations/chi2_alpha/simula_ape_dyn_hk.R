...
for(n in size){
    for(T in time){
        ...
        filename = sprintf("APEbc_dyn_chi2_N%g_T%g_ga%g_HK_IT%g",n, T, ga, nit)
        ...
        for(it in 1:nit){
            ...
            if(T<=8){
                ...
                while(bb<500){
                    ...
                    bmod_jk =  try(jackknife_logit(idp, yb, Xb, jk = TRUE, dyn = TRUE),silent=TRUE)
                ...
                SEJK[it,1] = se_ape_jk[1]
                APEJK[it,2] = mod_jk$ape_jk[2]
                SEJK[it,2] = se_ape_jk[2]
